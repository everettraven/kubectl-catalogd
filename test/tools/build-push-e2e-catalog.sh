...
kubectl create configmap -n "${namespace}" --from-file=test/testdata/test-catalog.Dockerfile kubectl-catalogd-e2e.dockerfile
kubectl create configmap -n "${namespace}" --from-file=test/testdata/test-catalog kubectl-catalogd-e2e.build-contents
  ...
  template:
    spec:
      ...
      volumes:
        - name: dockerfile
          configMap:
            name: kubectl-catalogd-e2e.dockerfile
            items:
              - key: test-catalog.Dockerfile
                path: test-catalog.Dockerfile
        - name: build-contents
          configMap:
            name: kubectl-catalogd-e2e.build-contents
EOF
